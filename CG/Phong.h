...
void scanlinepoly( point3d pts[], int n, float * depth, point3d color[],
                  point3d light[][3], point3d amb, point3d view, point3d coeff, float alpha, int l = 1 ) {
    ptcl cpts[10];
    ...
    cy[n] = color[0];
    double ymin = max( ( *min_element( y, y + n ) ) - 1, ( double )YMIN ), ymax = min( ( *max_element( y, y + n ) ) + 1, ( double )YMAX );
    int j = 0;
    point3d pt, col;
    while( ymin <= ymax ) {
        j = 0;
        ...
        for( int i = 0; i < j; i += 2 ) {
            ...
            for( cpt = xmi; cpt < xma; cpt += 1 ) {
                ...
                if( *( depth + a * STRIDE + b ) < zd ) {
                    pt.x = cpt, pt.y = ymin, pt.z = zd;
                    col = brightness(cd, pt, light, amb, view, coeff, alpha, l);
                    putpixel( int( cpt ), int( ymin ), COLOR( col.x, col.y, col.z ) );
                    *( depth + a * STRIDE + b ) = zd;
                }
                zd += incz;
                cd = cd + incc;
...
void drawgd( point3d** nrms, point3d** rpts, int u, int v, float * depth,
    ...
    for( int j = 0; j < u - 1; j++ ) {
        for( int k = 0; k < v - 1; k++ ) {
            ...
            point3d ppts[7] = { rpts[j][k], rpts[j + 1][k], rpts[j + 1][k + 1], rpts[j][k], rpts[j + 1][k + 1], rpts[j][k + 1], rpts[j][k]  };
            scanlinepoly( ppts, 3, ( float* ) depth, color, light, amb, view, coeff, alpha, n );
            scanlinepoly( ppts + 3, 3, ( float* ) depth, color + 3, light, amb, view, coeff, alpha, n ) ;
        }
    }
